# Technical Report — Predictive Maintenance & Anomaly Detection for Electromechanical Actuators
...
to t = 8.024s (after the fault, with a 0.024s latency) — the correct
causal relationship.
...
*Rolling RMS with 3-sigma threshold and alarm marker (top), FFT comparison
across healthy/friction-fault/thermal-fault windows (bottom).*
...
increases, from 0.086s at +10% down to 0.017s at +300%.*
...
## 6. Findings and limitations, stated plainly
...
- **Only one fault is independently swept and re-verified.** The thermal
  fault (t = 14s, +80% resistance) was confirmed to produce a physically
  correct current/speed signature during the initial model checkpoint, but
  the detection pipeline stops at the first alarm crossing per run — it
  does not separately flag a second, later fault event in the same
  simulation. Extending the detector to track and report multiple
  sequential fault events would be a natural next step.
