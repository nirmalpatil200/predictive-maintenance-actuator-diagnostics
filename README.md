...
├── models/
│   ├── build_model.m           # Builds the Simulink model via script (no GUI wiring)
│   ├── actuator_params.mat     # Saved model parameters (avoids workspace-state bugs)
│   └── Actuator_FaultInjection_Model.slx
├── scripts/
│   ├── run_predictive_diagnostics.m   # Single-run: RMS, threshold, FFT, dashboard
│   └── run_severity_sweep.m           # 9-level severity sweep verification
├── results/
...
build_model                    % builds and saves the Simulink model + params
run_predictive_diagnostics     % runs one simulation, detects the fault, plots the dashboard
run_severity_sweep             % re-runs detection across 9 fault severities
```
...
severity increases — a consistent, physically sensible trend across every
level tested.
...
- **Only the friction fault is swept and independently verified in depth.**
  The thermal fault (t = 14s) is injected and its current/speed signature
  was confirmed physically correct during initial checkpoint, but the
  detection pipeline stops at the first alarm crossing — it does not
  separately flag a second, distinct fault event later in the same run.
  Extending detection to handle multiple sequential faults would be a
  natural next step.
